...
# --------- pGWO-CSA (Improved GWO + Clonal Selection) ----------
def GWO(X, y, initial_best, n_agents=30, max_iter=50):
    ...
    for t in range(max_iter):
        ...
        # ---- Nonlinear control parameter a (Eq. 9) ----
        u = 2
        a = np.cos(np.pi * ((t / max_iter) ** u)) + 1
        ...
        for i in range(n_agents):
            ...
            # ---- Modified position update (Eq. 10) ----
            if i == sorted_idx[0]:  # alpha
                Xp = X1
            elif i == sorted_idx[1]:  # beta
                Xp = (X1 + X2) / 2
            else:  # others
                Xp = (X1 + X2 + X3) / 3
            ...
            # ---- CSA super-mutation mechanism ----
            fit_i = fitnesses[i]
            fit_min = np.min(fitnesses)
            fit_max = np.max(fitnesses)

            # Super-mutation coefficient (Eq. 11)
            if fit_max == fit_min:
                Sc = 0.1
            else:
                Sc = (fit_i - fit_min) / (fit_max - fit_min) + 0.1

            r3 = np.random.rand()

            if Sc > r3:
                # Mutation: apply GWO update again
                Xm = np.clip((X1 + X2 + X3) / 3, 0, 1)

                fit_Xp = fitness(binarize(Xp), X, y)
                fit_Xm = fitness(binarize(Xm), X, y)

                # Keep better individual (Eq. 12)
                new_wolves[i] = Xm if fit_Xm < fit_Xp else Xp
            else:
                new_wolves[i] = Xp
