...
def run(X, y):
    ...
    try:
        y_proba = clf.predict_proba(X_sel)
        if y_proba.shape[1] == 2:
            auc = roc_auc_score(y, y_proba[:, 1])
        else:
            auc = roc_auc_score(y, y_proba, multi_class='ovo', average='macro')
    except:
        auc = -1  # fallback if AUC can't be calculated (e.g., no proba available)

    print(f"Precision: {precision:.4f}")
    print(f"Recall: {recall:.4f}")
    print(f"F1 Score: {f1:.4f}")
    print(f"AUC-ROC: {auc:.4f}" if auc != -1 else "AUC-ROC: N/A")
