...
# --------- ADAPTIVE SCA ----------
def SCA_adaptive(X, y, initial_best, n_agents=20, max_iter=50):
    dim = X.shape[1]
    population = np.random.rand(n_agents, dim)
    population[0] = initial_best.astype(float)

    fitnesses = np.array([fitness(binarize(agent), X, y) for agent in population])
    ...
    for t in range(max_iter):
        r1 = 2 - t * (2 / max_iter)  # Linearly decreased exploration factor

        # Adaptive mechanism based on convergence speed:
        # If fitness hasn't improved much, reduce r1 faster (more exploitation)
        if t > 0 and abs(prev_best_fitness - best_fitness) < 1e-4:
            r1 *= 0.9
        ...
        for i in range(n_agents):
            for j in range(dim):
                ...
                if r4 < 0.5:
                    population[i, j] = population[i, j] + r1 * np.sin(r2) * abs(r3 * best_agent[j] - population[i, j])
                else:
                    population[i, j] = population[i, j] + r1 * np.cos(r2) * abs(r3 * best_agent[j] - population[i, j])

        population = np.clip(population, 0, 1)

        fitnesses = np.array([fitness(binarize(agent), X, y) for agent in population])
        curr_best_idx = np.argmin(fitnesses)
        if fitnesses[curr_best_idx] < best_fitness:
            best_fitness = fitnesses[curr_best_idx]
            best_agent = population[curr_best_idx].copy()
...
def run(X, y):
    ...
    # Phase 2: Adaptive SCA
    sca_best = SCA_adaptive(X, y, initial_best=full_best)

    final_fitness = fitness(sca_best, X, y)
    acc = 1 - final_fitness
    feature_count = np.sum(sca_best)
    elapsed = time.time() - start

    return sca_best, acc, feature_count, elapsed
