...
def GWO(X, y, initial_best, n_agents=30, max_iter=50):
    ...
    for t in range(max_iter):
        ...
        for i in range(n_agents):
            ...
            # ===== Candidate 1: Standard GWO movement (Eq. 6–8) =====
            ...
            Xi_gwo = np.clip((X1 + X2 + X3) / 3.0, 0, 1)

            # ===== Candidate 2: DLH search (Eq. 10–12) =====
            R = np.linalg.norm(Xi - Xi_gwo)

            # Neighborhood construction
            distances = np.linalg.norm(wolves - Xi, axis=1)
            neighbors_idx = np.where(distances <= R)[0]

            if len(neighbors_idx) == 0:
                neighbors_idx = np.array([np.random.randint(n_agents)])

            Xn = wolves[np.random.choice(neighbors_idx)]
            Xr = wolves[np.random.randint(n_agents)]

            Xi_dlh = Xi + np.random.rand(dim) * (Xn - Xr)
            Xi_dlh = np.clip(Xi_dlh, 0, 1)

            # ===== Selection phase (Eq. 13) =====
            fit_gwo = fitness(binarize(Xi_gwo), X, y)
            fit_dlh = fitness(binarize(Xi_dlh), X, y)

            if fit_gwo < fit_dlh:
                candidate = Xi_gwo
                candidate_fit = fit_gwo
            else:
                candidate = Xi_dlh
                candidate_fit = fit_dlh

            # ===== Update phase =====
            if candidate_fit < fitnesses[i]:
                new_wolves[i] = candidate
