...
# --------- EBGWO (Elite Balance Grey Wolf Optimizer) ----------
def GWO(X, y, initial_best, n_agents=30, max_iter=50, ST=0.2):
    dim = X.shape[1]
    ...
    for t in range(max_iter):
        ...
        current3 = np.array([alpha, beta, delta])

        if t == 0:
            candidate3 = current3.copy()
        else:
            pool = np.vstack((parent3, current3))
            pool_fitness = np.array(
                [fitness(binarize(p), X, y) for p in pool]
            )
            best_idx = np.argsort(pool_fitness)[:3]
            candidate3 = pool[best_idx]

        parent3 = candidate3.copy()
        ...
        for i in range(n_agents):
            ...
            if np.random.rand() < ST:
                # ===== Balance search (exploration) =====
                Xc1, Xc2 = candidate3[0], candidate3[1]
                Xr = wolves[np.random.randint(n_agents)]

                r1, r2 = np.random.rand(dim), np.random.rand(dim)
                A1 = 2 * a * r1 - a
                C1 = 2 * r2
                D1 = np.abs(C1 * Xc1 - wolves[i])
                X1 = Xc1 - A1 * D1

                r1, r2 = np.random.rand(dim), np.random.rand(dim)
                A2 = 2 * a * r1 - a
                C2 = 2 * r2
                D2 = np.abs(C2 * Xc2 - wolves[i])
                X2 = Xc2 - A2 * D2

                r1, r2 = np.random.rand(dim), np.random.rand(dim)
                A3 = 2 * a * r1 - a
                C3 = 2 * r2
                D3 = np.abs(C3 * Xr - wolves[i])
                X3 = Xr - A3 * D3
                ...
                A1 = 2 * a * r1 - a
                C1 = 2 * r2
                D1 = np.abs(C1 * alpha - wolves[i])
                X1 = alpha - A1 * D1

                r1, r2 = np.random.rand(dim), np.random.rand(dim)
                A2 = 2 * a * r1 - a
                C2 = 2 * r2
                D2 = np.abs(C2 * beta - wolves[i])
                X2 = beta - A2 * D2

                r1, r2 = np.random.rand(dim), np.random.rand(dim)
                A3 = 2 * a * r1 - a
                C3 = 2 * r2
                D3 = np.abs(C3 * delta - wolves[i])
                X3 = delta - A3 * D3
