...
# --------- IAGWO (Improved Adaptive Grey Wolf Optimizer) ----------
def GWO(X, y, initial_best, n_agents=30, max_iter=50):
    dim = X.shape[1]

    # --- Initialize wolves and velocities (PSO mechanism) ---
    ...
    # IMF inertia weight range (paper)
    w_max, w_min = 0.9, 0.3
    theta = 0.5  # sigmoid exponent
    ...
    for t in range(max_iter):
        ...
        # ---- IMF inertia weight (nonlinear decay) ----
        w = w_max - (w_max - w_min) * (t / max_iter)

        # ---- Standard GWO control parameter ----
        a = 2 - 2 * (t / max_iter)

        fave = np.mean(fitnesses)
        ...
        for i in range(n_agents):
            ...
            velocities[i] = (
                w * velocities[i]
                + np.random.rand(dim) * (alpha - wolves[i])
            )

            Xpso = wolves[i] + velocities[i]

            # ---- Combine PSO + GWO search ----
            Xnew = 0.5 * Xgwo + 0.5 * Xpso

            # ---- Aggregation coefficient ----
            if fave == 0:
                agg = 1.0
            else:
                agg = fitnesses[i] / fave

            # ---- Sigmoid adaptive scaling ----
            scale = 1 / (1 + np.exp(-agg ** theta))

            Xnew = wolves[i] + scale * (Xnew - wolves[i])
