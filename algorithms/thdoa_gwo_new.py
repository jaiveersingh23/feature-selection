...
# --------- FINAL GREEDY PRUNING ----------
def greedy_prune(solution, X, y, beta=0.9, tolerance=0.001):
    """Greedily prune features without harming accuracy."""
    pruned_solution = solution.copy()
    base_fitness = fitness(pruned_solution, X, y, beta)
    
    selected_indices = np.where(pruned_solution == 1)[0]
    for idx in selected_indices:
        pruned_solution[idx] = 0
        new_fitness = fitness(pruned_solution, X, y, beta)
        
        # If error did not increase beyond tolerance, keep feature dropped
        if new_fitness <= base_fitness + tolerance:
            base_fitness = new_fitness
        else:
            pruned_solution[idx] = 1  # Revert
...
def run(X, y):
    ...
    full_best[selected_indices] = gwo_best

    # Final greedy pruning
    pruned_best = greedy_prune(full_best, X, y)

    # Final results after pruning
