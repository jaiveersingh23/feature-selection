...
def HOA(X, y, initial_best, n_agents=20, max_iter=50):
    ...
    horses[0] = initial_best  # Inject best from previous phase
...
def run(X, y):
    ...
    # Phase 3: HOA on GWO-best
    hoa_best = HOA(X_reduced, y, initial_best=gwo_best)

    # Restore full vector
    full_best = np.zeros(X.shape[1], dtype=int)
    full_best[selected_indices] = hoa_best
